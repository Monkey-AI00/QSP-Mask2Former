...
# 画图
def draw_result(src, temp, match_points):
    cv.rectangle(src, match_points['point'],
                  (match_points['point'][0] + temp.shape[1], match_points['point'][1] + temp.shape[0]),
                  (0, 255, 0), 2)
    
    # 绘制中心点
    center = [0,0]
    top = match_points['point']
    center[0]=top[0]+temp.shape[1]//2
    center[1]=top[1]+temp.shape[0]//2
    cv.circle(src, center, 5, (0, 0, 255), -1)  # 红色中心点

    # 标注中心坐标
    cv.putText(src, f'({center[0]}, {center[1]})', (center[0] + 10, center[1] - 10),
...
def get_realsense(src, temp):
    ModelImage = temp
    SearchImage = src
    ModelImage_edge = cv.GaussianBlur(ModelImage, (1, 1), 0)
    ModelImage_edge = cv.Canny(ModelImage_edge, 30, 200, apertureSize=3)  #10 200
    # contours, _ = cv.findContours(ModelImage_edge, cv.RETR_CCOMP, cv.CHAIN_APPROX_SIMPLE)
    # cv.drawContours(src, contours, 1, (0, 255, 0), 2)
    # cv.imshow('result', src)
    SearchImage_edge = cv.GaussianBlur(SearchImage, (1,1), 0)
 
    (h1, w1) = SearchImage_edge.shape[:2]
    SearchImage_edge = cv.Canny(SearchImage_edge, 10, 180, apertureSize=3)      #10 180
    ...
    print('匹配的最优区域的中心坐标为：' + str(center))
    ...
    cv.waitKey()
    draw_result(SearchImage, ModelImage_edge, match_points)
    return match_points


src = cv.imread('/home/iqr/pan/calibrate/image2/2.jpg')
temp = cv.imread('/home/iqr/pan/calibrate/image2/1_model2.jpg')
