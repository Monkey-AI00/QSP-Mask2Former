...
def force_control(setpoint, force_con, action, pid, desired_x, desired_y):
    ...
    # 抽象的计算方法，哈哈哈哈哈哈，实验结果，后续：改变位姿之后，又失败了，难绷
    # pos[0] = -desired_x + p_now[0]
    # pos[1] = desired_y - p_now[1]
    # a = desired_x - p_now[0]
    # b = desired_y - p_now[1]
    # print('增量x', a)
    # print('增量y', b)
...
if __name__ == '__main__':
    ...
    print(F_zero)
    setpoint1 = [F_zero[0], F_zero[1], F_zero[2]+8 , F_zero[3], F_zero[4], F_zero[5]]
    # action = [0,0,-0.002,0,0,0] 
    action = [0, 0, 0, 0, 0, 0]

   # 参数定义
    z = np.linspace(0, 18, 6000)  # z 生成范围
    r = 0.0001  # 半径缩放因子
    ...
    for i in range(len(z)):
        ...
        # 计算轨迹
        x = z[i] * r * np.cos(np.pi * 5 * z[i])
        y = z[i] * r * np.sin(np.pi * 5 * z[i]) * 1.5
        print('x',x)
        # print(y)
        desired_x = startpos[0] + x
        desired_y = startpos[1] + y

        f, pos = force_control(setpoint1, force_con, action, pid, desired_x, desired_y)
