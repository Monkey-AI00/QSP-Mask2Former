...
def force_control(setpoint, force_con, action, pid, desired_y, desired_z):
    ...
    # pos[2] = -desired_z + p_now[2]
    print('PID输出增量:',pos[0])
    pos_keep = rtde_c.poseTrans(p_now, pos)  # poseTrans 方法用于将一个相对变换应用到给定的位姿上，生成一个新的位姿。
    # ?pos_keep[0] = pos[0] + p_now[0]
    print('新位置:',pos_keep[0])
    pos_keep[1] = desired_y
    pos_keep[2] = desired_z
    print(desired_y)
    for y in range(100):
        servo_move(pos_keep)  #通过 servoL 命令，将机械臂通过直线运动（Linear motion）逐步移动到新的目标位置 pos_keep。这个过程重复 50 次以保持平稳过渡。
...
if __name__ == '__main__':
    ...
    action = [-0.000, 0, 0, 0, 0, 0]

   # 参数定义
    c = np.linspace(0, 18, 6000)  #生成范围
    r = 0.0001  # 半径缩放因子
