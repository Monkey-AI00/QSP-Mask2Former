...
if __name__ == '__main__':
    ...
    contact_threshold = F_zero[0] + 9  # 定义接触力的阈值
    # 定义力突变阈值
    x_force_change_threshold = 5  # 你可以调整这个值
    ...
    for i in range(len(c)):
        ...
        if f_now[0] > contact_threshold:
            contact_detected = True  # 标记为已接触         有点问题
            print("物体接触检测成功，开始YZ面螺旋运动")

        if not contact_detected:
            # **接触前：仅在X方向运动**
            desired_y = startpos[1]
            desired_z = startpos[2]
            # print(desired_y)
            # print(desired_z)
            ...
            # **接触后：YZ面螺旋运动**
            # desired_y = startpos[1] = c[i] * r * np.cos(np.pi * 5 * c[i])
            # desired_z = startpos[2] = c[i] * r * np.sin(np.pi * 5 * c[i]) * 1.5
            desired_y = c[i] * r * np.cos(np.pi * 5 * c[i])
            desired_z = c[i] * r * np.sin(np.pi * 5 * c[i]) * 1.5
            print(desired_y)
            print(desired_z)
            f, pos = force_control(setpoint1, force_con, action, pid, desired_y, desired_z)
