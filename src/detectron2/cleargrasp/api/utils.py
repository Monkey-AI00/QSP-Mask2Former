...
# 可选依赖：只有读写 .exr 时才需要（推理/深度补全主链路不强依赖）
try:  # pragma: no cover
...
def exr_loader(EXR_PATH, ndim=3):
    ...
    if OpenEXR is None or Imath is None:
        raise ModuleNotFoundError("缺少 OpenEXR/Imath 依赖，无法读取 EXR。请安装 openexrpython 或跳过 EXR 相关功能。")
...
def exr_saver(EXR_PATH, ndarr, ndim=3):
    ...
    '''
    if OpenEXR is None or Imath is None:
        raise ModuleNotFoundError("缺少 OpenEXR/Imath 依赖，无法写入 EXR。请安装 openexrpython 或跳过 EXR 相关功能。")
    if ndim == 3:
        # Check params
