    ...
    def __init__(self, ip, port, *args):
        ...
        if self.port == 29999 or self.port == 30004 or self.port == 30005:
            try:
                ...
                self.socket_dobot.settimeout(self.connect_timeout_s)
    ...
    def send_data(self, string):
       # self.log(f"Send to {self.ip}:{self.port}: {string}")
        try:
            ...
            print(e)
            # reConnect() 内部已经做了有限重试，避免此处再套一层重试导致长时间“假卡住”
            try:
                ...
                raise ConnectionError(
                    f"[dobot_api] send_data failed after reconnect for {self.ip}:{self.port}"
                ) from retry_e
    ...
    def wait_reply(self):
        ...
        try:
            ...
            try:
                ...
                raise ConnectionError(
                    f"[dobot_api] wait_reply failed and reconnect failed for {self.ip}:{self.port}"
                ) from reconnect_e
        if len(data) == 0:
            data_str = data
        else:
    ...
    def reConnect(self, ip, port):
        ...
        for attempt in range(1, self.reconnect_max_attempts + 1):
            try:
                ...
                socket_dobot.settimeout(self.connect_timeout_s)
                socket_dobot.connect((ip, port))
                socket_dobot.settimeout(None)
                if port in (30004, 30005):
                    socket_dobot.setsockopt(socket.SOL_SOCKET, socket.SO_RCVBUF, 144000)
                return socket_dobot
            except Exception as e:
                last_error = e
                print(
                    f"[dobot_api] reconnect failed ({attempt}/{self.reconnect_max_attempts}) "
                    f"{ip}:{port}: {type(e).__name__}: {e}"
                )
                sleep(self.reconnect_interval_s)
        raise ConnectionError(
            f"[dobot_api] reconnect exhausted for {ip}:{port} "
            f"after {self.reconnect_max_attempts} attempts"
        ) from last_error
