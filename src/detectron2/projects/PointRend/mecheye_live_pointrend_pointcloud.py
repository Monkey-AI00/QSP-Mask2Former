...
def instances_to_binary_mask_u8_pc(
    ...
    *,
    ...
    # 合并规则（满足其一即可）：
    # - bbox IoU >= iou_thresh
    # - 与锚点 mask（可选膨胀）有像素级接触/重叠（更适合你现在这种被拆成多个实例的情况）
    scores = instances.scores.numpy() if hasattr(instances, "scores") else None
    best_idx = int(np.argmax(scores)) if scores is not None and scores.size > 0 else 0
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=25,
        help="pc-mask-mode=iou 时，锚点实例 mask 的膨胀核大小（像素）。用于把相邻碎片（例如 cls0 60% 那块）并入同一目标。0 表示关闭。建议 11~51。",
    )
    ...
    parser.add_argument("--win", default="Mech-Eye Live (PointRend)", help="窗口标题")
    ...
    parser.add_argument(
        "--show-depth",
        action="store_true",
        help="实时可视化 raw depth：显示 raw depth 的彩色渲染图",
    )
    ...
    meta_name = "__mecheye_live_pointrend__"
    ...
    if bool(args.cleargrasp):
        if not str(args.cleargrasp_normals_weights).strip() or not str(args.cleargrasp_outlines_weights).strip() or not str(args.cleargrasp_depth2depth_exe).strip():
            ...
            try:
                ...
                # 注意：ClearGrasp 内部会把输入 resize 到 out_w/out_h
                out_w, out_h = int(args.cleargrasp_out_w), int(args.cleargrasp_out_h)
                # 输入图像尺寸还未知（采第一帧时才能知道），这里延后在按 s 时再设置 scaled K
                depthcomplete = {
    ...
    try:
        while True:
            ...
            if do_infer:
                outputs = predictor(color_bgr)
                instances = outputs["instances"].to("cpu")
                mask_u8 = instances_to_binary_mask_u8(instances, mask_mode=str(args.mask_mode))
                mask_u8 = _morph_mask(mask_u8, close=int(args.mask_close), dilate=int(args.mask_dilate), erode=int(args.mask_erode))
                if bool(args.invert_mask):
                    mask_u8 = 255 - mask_u8

                # 可视化（复用 inference_plug.py 的风格）
                v = Visualizer(
                    color_bgr[:, :, ::-1],
                    metadata=metadata,
                    scale=1.0,
                ...
                last_vis = out.get_image()[:, :, ::-1]  # RGB->BGR
            ...
            if key == ord("s"):
                ...
                # - mask_u8_pc：用于点云导出与 cleargrasp 点云过滤（受 --pc-mask-mode 控制，默认 maxscore）
                mask_u8_vis = instances_to_binary_mask_u8(instances, mask_mode=str(args.mask_mode))
                mask_u8_vis = _morph_mask(
                    mask_u8_vis,
                    close=int(args.mask_close),
                ...
                mask_path = Path(save_dir) / f"pointrend_mask_{ts}.png"
                vis_path = Path(save_dir) / f"pointrend_vis_{ts}.png"
                ply_path = Path(save_dir) / f"mask_pointcloud_{ts}.ply"
                    ...
                    if depthcomplete is None:
                        ...
                        try:
                            ...
                            if "model" not in depthcomplete:
                                ...
                                fx0, fy0, cx0, cy0 = float(depthcomplete["fx"]), float(depthcomplete["fy"]), float(depthcomplete["cx"]), float(depthcomplete["cy"])
                                out_w, out_h = int(depthcomplete["out_w"]), int(depthcomplete["out_h"])
                                fx2, fy2, cx2, cy2 = _scale_k_for_resize(fx0, fy0, cx0, cy0, in_w=W, in_h=H, out_w=out_w, out_h=out_h)
                            ...
                            out_depth_m_up = cv2.resize(out_depth_m_small, (depth_m.shape[1], depth_m.shape[0]), interpolation=cv2.INTER_NEAREST).astype(np.float32, copy=False)
                            ...
                            cv2.imwrite(str(depth_completed_path), _depth_m_to_u16(completed_m, unit=str(args.cleargrasp_depth_unit)))  # type: ignore[arg-type]

                            # 只输出插头区域点云：使用 mask_u8（已受 --invert-mask 影响）
                            fx, fy, cx, cy = _get_depth_k_from_mecheye_intrinsics(intrinsics)
                            ...
                            print(f"[cleargrasp] saved: {depth_completed_path.name}, {ply_completed_path.name} (points={int(xyz.shape[0])})")
                        except Exception as e:
