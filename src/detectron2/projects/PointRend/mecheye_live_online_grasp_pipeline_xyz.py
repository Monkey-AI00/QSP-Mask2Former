...
class OnlineGraspPipeline:
    def __init__(self, args):
        ...
        self._last_gripper_feedback_ts: float = 0.0
        self._last_gripper_state: Optional[int] = None
        self._hotkey_help_printed: bool = False
        self._last_final_pose_mm_deg: Optional[np.ndarray] = None
    ...
    @staticmethod
    def _format_gripper_status(status: dict[str, Any]) -> str:
    ...
    def _close_gripper_after_grasp(self, *, context: str) -> None:
        if self.executor is None:
            return
        if not bool(self.args.gripper_enable):
            return
        try:
            print(f"[gripper] 到达抓取点后闭合夹爪并停留等待（{context}）")
            self.executor.close_gripper()
            timeout_s = max(0.0, float(getattr(self.args, "gripper_close_feedback_timeout", 2.0)))
            poll_s = max(0.05, float(getattr(self.args, "gripper_feedback_interval", 0.5)))
            if timeout_s > 0.0:
                t0 = time.time()
                last_status_line = ""
                got_object = False
                while (time.time() - t0) < timeout_s:
                    status = self.executor.get_gripper_status()
                    if status is not None:
                        grip_state = int(status.get("grip_state", -1))
                        self._last_gripper_state = grip_state
                        line = self._format_gripper_status(status)
    ...
    def _wait_grip2_then_back_to_p1(self) -> None:
        ...
        poll_s = max(0.05, float(getattr(self.args, "gripper_feedback_interval", 0.5)))
        t0 = time.time()
        grip2 = (self._last_gripper_state == 2)
        while (not grip2) and ((time.time() - t0) < timeout_s):
            status = self.executor.get_gripper_status()
            if status is not None:
                self._last_gripper_state = int(status.get("grip_state", -1))
                print(f"[gripper][feedback][wait] {self._format_gripper_status(status)}")
                if self._last_gripper_state == 2:
                    grip2 = True
                    break
            time.sleep(poll_s)
        if not grip2:
        ...
        self.executor.movj_joint(p1_joint, "post_grasp_back_to_p1")
    ...
    def _handle_gripper_hotkey(self, key: int) -> None:
        ...
        if key == ord("p"):
            try:
                ...
                self.executor.movj_joint(p1_joint, "hotkey_back_to_p1")
            except Exception as e:
    ...
    def _maybe_print_gripper_stroke(self) -> None:
        if self.executor is None or (not bool(self.args.gripper_enable)):
            return
        interval_s = float(getattr(self.args, "gripper_feedback_interval", 0.5))
        if interval_s <= 0.0:
            return
        now = time.time()
        if (now - float(self._last_gripper_feedback_ts)) < interval_s:
            return
        self._last_gripper_feedback_ts = now
        status = self.executor.get_gripper_status()
        if status is not None:
            print(f"[gripper][feedback] {self._format_gripper_status(status)}")
    ...
    def _cache_final_pose_and_log(self, pose_mm_deg: np.ndarray, *, context: str) -> np.ndarray:
        ...
        self._last_final_pose_mm_deg = pose.copy()
    ...
    def _go_home_then_revisit_final_pose(self, final_pose_mm_deg: np.ndarray, *, context: str) -> None:
        ...
        if offset_text:
            ...
            # 兼容旧参数：preset_z = final_z - retreat_mm
    ...
    def _run_runtime_probe_and_execute(self, pose_pre: np.ndarray, pose_grasp: np.ndarray) -> bool:
        ...
        if bool(self.args.prefer_api_ik_joint_execution):
            ...
            if bool(pre_api.get("ok", False)) and bool(grasp_api.get("ok", False)):
                ...
                self._close_gripper_after_grasp(context="runtime_joint_solution")
        ...
        self._close_gripper_after_grasp(context="runtime_direct_pose_fallback")
